...
public class StudentServiceImpl extends ServiceImpl<StudentMapper, Student> implements IStudentService {
    ...
    public String findNextStuId(String stuIdPrefix) {
        int nextNum=1;
        String maxStuId=studentMapper.findMaxStuId(stuIdPrefix);//2019 012 01
        if (null==maxStuId){
            return stuIdPrefix+"01";
        }
        nextNum=Integer.parseInt(maxStuId.substring(7, 9))+1;//02=>2
        return stuIdPrefix+String.format("%02d",Integer.valueOf(nextNum));//前缀+02
    }
